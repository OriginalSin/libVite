    ...
    pointToCanvas: function(attr) { // Точку в canvas
        ...
        if (image) {
            ...
            if (gmx.transformFlag) {	// TODO:
				...
				px1sx = Math.round(px1sx);
				py1sy = Math.round(py1sy);
				if (style.rotateRes) {
...
*/
    ...
    */
	...
	getFeaturesFromGeometries: function (arr) {
		if (!L.Util.isArray(arr)) { arr = [arr]; }
		var out = arr.map(function (it) {
			return L.gmxUtil.geometryToGeoJSON(it, true, true);
		});
		return out;
    },
    ...
    parseBalloonTemplate: function(str, options) {
        ...
        if (matches) {
            ...
            for (var i = 0, len = matches.length; i < len; i++) {
                ...
                } else if (keyLC === 'geometry') {
                    res = L.gmxUtil.getFeaturesFromGeometries(geometries).map(JSON.stringify).join('\n');
					res = '<b>Геометрия</b>:<br><textarea class="geoJson" rows="4" disabled style="height: 100px;">' + res + '</textarea>';
                }
                str = str.replace(key1, res);
            }
        }
...
L.extend(L.gmxUtil, {
    ...
    getFeaturesFromGeometries: gmxAPIutils.getFeaturesFromGeometries,
...
var DataManager = L.Class.extend({
    ...
    options: {
		...
		needBbox: true,               		// flag for get tiles list by BBOX
...
var ProjectiveImageWebGL = L.Class.extend({
    ...
    getCanvas: function (attr) {
        ...
        var glOpts = {
            ...
            gl = offscreen.getContext('webgl', glOpts) || offscreen.getContext('experimental-webgl', glOpts);
        if (!gl) { return; }
        var glResources = this._setupGlContext(gl);
        if (!glResources) { return; }

        glResources.canvas = this.canvas = offscreen;
