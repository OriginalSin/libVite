...
const Feature = L.LayerGroup.extend({
    ...
    _fireEvent: function (name, options) {
        ...
        if (name === 'addPoly') {
			var it = new Ring(this, options.latLngsArr, {ring: true, editable: true});
			this.addLayer(it);
			this.rings.push({ring: it, holes: []});
			this.options.type = 'MultiPolygon';
            return;
        }
        if (name === 'addLine') {
			var it = new Ring(this, options.latLngsArr, {ring: true, editable: true});
			this.addLayer(it);
			this.rings.push({ring: it, holes: []});
			this.options.type = 'MultiPolyline';
            return;
        }
    ...
    getOptions: function () {
        ...
        var res = Utils.getNotDefaults(data, Utils.defaultStyles);
        ...
        if (options.type === 'Point') {
            var opt = Utils.getNotDefaults(this._obj.options, Utils.defaultStyles.markerStyle.options);
            if (Object.keys(opt).length) { res.options = opt; }
            opt = Utils.getNotDefaults(this._obj.options.icon.options, Utils.defaultStyles.markerStyle.options.icon);
    ...
    _latlngsAddShift: function (latlngs, shiftPixel) {
        ...
        for (var i = 0, len = latlngs.length; i < len; i++) {
            arr.push(Utils.getShiftLatlng(latlngs[i], this._map, shiftPixel));
        }
        return arr;
    },
    ...
    _chkPolygon: function (arr) {
		if (L.gmxUtil.isClockwise(arr[0])) {
			arr[0] = arr[0].reverse();
		}
		for (var i = 1, len = arr.length; i < len; i++) {
			if (!L.gmxUtil.isClockwise(arr[i])) {
				arr[i] = arr[i].reverse();
			}
		}
		return arr;
    ...
    _toGeoJSON: function (withoutShift) {
        ...
        if (!this.options.editable || type === 'Point') {
            ...
            if (obj instanceof L.GeoJSON) {
                obj = Utils._getLastObject(obj).getLayers()[0];
            }
		...
		if (coords) {
			...
			if (type === 'MultiPolygon') {
				coords = coords.map(arr => {
					var tt = _this._chkPolygon(arr);
					return tt;
				});
			} else if (type === 'Polygon') {
				if (L.gmxUtil.isClockwise(coords[0])) {
					coords[0] = coords[0].reverse();
				}
				for (var i = 1, len = coords.length; i < len; i++) {
					if (!L.gmxUtil.isClockwise(coords[i])) {
    ...
    _initialize: function (parent, obj) {
        ...
        if (this.options.editable) {
			...
			if (Utils.isOldVersion) {
				arr = obj.getLayers ? Utils._getLastObject(obj).getLayers() : [obj];
			} else {
				arr = obj.getLayers ? Utils._getLastObject(obj) : [obj];
					...
					if (ring && (ring.downObject || !down)) {
                       var mapOpt = my._map ? my._map.options : {},
                        ...
                        if (type === 'Area') {
                            ...
                            var downAttr = Utils.getDownType.call(my, ev, my._map, my),
        ...
        marker
            ...
            .on('drag', function(ev) {
				if (ev.originalEvent && ev.originalEvent.ctrlKey) {
					marker.setLatLng(Utils.snapPoint(marker.getLatLng(), marker, _map));
				}
