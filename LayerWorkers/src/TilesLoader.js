...
// console.log('load', pars);
	// return new Promise((resolve) => {
		let tilesOrder = pars.tilesOrder,
			...
			LayerName = pars.name,
			pb = pars.tiles,
			hostName = pars.hostName || Utils.HOST,
		...
		for (let i = 0, len = pb.length; i < len; i += tilesOrder.length) {
			...
			if (pars.tilesPromise[tkey]) {
				...
				tilesPromise[tkey] = Requests.getTileJson({
					...
					paramsArr: [tHash, { LayerName, sw: 1, r: 'j', ModeKey: 'tile' }]
				}).then(json => {
		...
		// return out;
		return Promise.all(Object.values(tilesPromise));
		/*
		Promise.all(Object.values(tilesPromise)).then(out => {
			// resolve(out);
			resolve({cmdNum, tiles: out});
		});
		*/
	// });
};
